...
# DeadDrop terminal client — fileless bootstrap.
# It downloads a tiny static binary into RAM (/dev/shm), runs it, and deletes
# it on exit. Nothing is written to persistent disk. The binary speaks the same
# end-to-end-encrypted protocol as the web app; the server never sees plaintext.
#
# Inspect before running:   curl %[1]s/cli
# Run:                      curl %[1]s/cli | sh
